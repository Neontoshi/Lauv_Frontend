...
const AudiobooksPage: React.FC = () => {
    ...
    try {
      ...
      const filtered = (yt || [])
        ...
        .map((r: any) => ({
          ...
          emoji: "📖",
          grad: "linear-gradient(135deg, #8B4513, #5C2E00)",
          bpm: 0,
          key: "—",
          plays: 0,
          liked: false,
  ...
  const handleSearch = (query: string) => {
    ...
    searchTimeout.current = window.setTimeout(() => {
      loadCategory(`${query} audiobook full`);
    }, 400);
  };
  ...
  return (
    ...
    >
      <style>{`
        .ab-header { flex-shrink: 0; padding: 1.5rem 2rem 0; background: var(--bg); }
        .ab-eyebrow { font-family: 'DM Mono', monospace; font-size: 10px; letter-spacing: 0.25em; text-transform: uppercase; color: var(--accent); margin-bottom: 4px; }
        .ab-title { font-family: 'Syne', sans-serif; font-size: 2rem; font-weight: 800; letter-spacing: -0.03em; line-height: 0.92; margin: 0; }
        .ab-tape { display: flex; gap: 0; overflow-x: auto; scrollbar-width: none; border-bottom: 1px solid var(--border); margin-top: 0.75rem; }
        .ab-tape::-webkit-scrollbar { display: none; }
        .ab-chip { flex-shrink: 0; padding: 8px 16px; background: transparent; border: none; border-bottom: 2px solid transparent; margin-bottom: -1px; color: var(--text3); font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 600; cursor: pointer; transition: color 0.15s, border-color 0.15s; white-space: nowrap; }
        .ab-chip:hover { color: var(--text2); }
        .ab-chip.active { color: var(--text); border-bottom-color: var(--accent2); }
        .ab-search { display: flex; align-items: center; gap: 8px; background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 8px 12px; margin: 0.75rem 0; }
        .ab-search:focus-within { border-color: var(--accent); }
        .ab-search input { background: none; border: none; color: var(--text); font-family: 'DM Mono',monospace; font-size: 12px; outline: none; width: 100%; }
        .ab-search input::placeholder { color: var(--text3); }
        .ab-content { flex: 1; overflow-y: auto; padding: 0 2rem 6rem; }
      `}</style>
