...
const TRENDING_QUERIES = [
  { label: "Hot Right Now", query: "official music video" },
  { label: "Top Charts", query: "top songs this week" },
  { label: "New Music", query: "new music this week official" },
  { label: "Hip Hop", query: "best hip hop 2025" },
  { label: "Electronic", query: "best electronic music 2025" },
  { label: "R&B", query: "best r&b songs 2025" },
  { label: "Alternative", query: "best alternative music 2025" },
  { label: "Afrobeats", query: "best afrobeats 2025" },
];
...
const ExplorePage: React.FC = () => {
  ...
  useEffect(() => {
    loadTrending(TRENDING_QUERIES[activeQuery].query);
  }, [activeQuery]);

  const loadTrending = async (query: string) => {
    setLoading(true);
    setYtResults([]);
    setScResults([]);
    try {
  ...
  const allSongs = [...ytResults, ...scResults];
  ...
  const handleLike = async (e: React.MouseEvent, song: Song) => {
    ...
    try {
      ...
      // Force re-render to reflect like state
      setYtResults([...ytResults]);
      setScResults([...scResults]);
    } catch (err) {
      console.error("Failed to toggle like:", err);
      toggleLike(song.id); // revert optimistic update
    }
  };
    ...
    <div className="ex-root">
      <style>{`
        ...
        .ex-now-strip {
          ...
          animation: exFadeIn 0.3s ease both;
        }
        ...
        .ex-play-circle:hover { transform: scale(1.07); opacity: 0.9; }
        ...
        .ex-card {
          ...
          border: 1px solid var(--border);
          cursor: pointer;
          animation: exFadeUp 0.35s ease both;
        ...
        .ex-card:hover .ex-like-btn,
        .ex-card.is-active .ex-like-btn {
          opacity: 1;
        }
        ...
        .ex-card.is-active .ex-card-overlay {
          opacity: 1;
        }
        ...
        .ex-skeleton {
          ...
          background: var(--surface);
          border: 1px solid var(--border);
          animation: exSkPulse 1.6s ease-in-out infinite;
        }
      ...
      <div className="ex-inner">
        ...
        {!loading && allSongs.length > 0 && (
          <div className="ex-now-strip">
            ...
            <div className="ex-strip-label">
              <div className="ex-strip-title">
                {TRENDING_QUERIES[activeQuery].label}
              </div>
              <div className="ex-strip-count">{allSongs.length} tracks</div>
            </div>
            <div className="ex-src-pills">
              {ytResults.length > 0 && (
                <span className="ex-src-pill yt">YT · {ytResults.length}</span>
              )}
              {scResults.length > 0 && (
                <span className="ex-src-pill sc">SC · {scResults.length}</span>
              )}
            </div>
          </div>
        )}
        ...
        {/* Content */}
        {loading ? (
          <div className="ex-grid" style={{ marginTop: "1.5rem" }}>
